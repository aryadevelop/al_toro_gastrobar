...
| DTOs | `LoginRequest`, `RegisterRequest`, `RefreshTokenRequest`, `AuthResponse`, `AuthUserResponse`, `RegisterResponse` |
...
| `ClienteController` | Endpoints `/api/clientes`: consulta de puntos propios (CLIENTE), búsqueda por fragmento de nombre (CAJERO/ADMIN), canje de puntos |
| `EmpleadoController` | Endpoints `/api/empleados`: crear empleado, listar con filtros por rol/estado/nombre, cambiar estado |
| DTOs | `UpdateClienteRequest/Response`, `ChangePasswordRequest/Response`, `ClientePuntosResponse`, `ClienteBusquedaResponse`, `CrearEmpleadoRequest`, `EmpleadoResponse`, `EmpleadoListadoResponse` |
...
| DTOs | `CrearReservaRequest`, `ModificarReservaRequest`, `RegistrarAbonoRequest`, `ReservaResponse`, `ReservaDetalleResponse`, `DisponibilidadResponse`, `ConfirmarReservaResponse`, `CancelarReservaResponse`, `ModificarReservaResponse`, `ResumenPagoResponse`, `RegistrarAbonoResponse`, `MarcarInasistenciaResponse` |
...
| `ReservaMapper` | Construye respuestas fusionando pares COCINA+BARRA en un único ítem con campo `bebida` |
...
| DTOs | `AsignarMesaRequest`, `AgregarItemRequest`, `ModificarItemRequest`, `BorradorComandaResponse`, `MapaMesasResponse`, `MesaAsignadaResponse`, `TableroProduccionResponse`, `ComandaProduccionDetalleResponse`, `EstadoVisitaResponse`, `VisitaDetalleResponse`, `VisitaResumenResponse` |
...
| Mappers | `MesaMapper`, `ComandaBorradorMapper`, `VisitaMapper`, `VisitaEstadoMapper` |
...
| DTOs | `CategoriaCartaResponse`, `MenuEspecialResponse`, `ProductoBusquedaResponse`, `ProductoInventarioResponse`, `CambioEstadoRequest`, `CambioEstadoResponse`, `CambioEstadoImplicacionesResponse`, `AjusteInventarioRequest`, `AjusteInventarioResponse`, `ItemAjusteInventarioResponse` |
| `ProductoService` | Construye la carta agrupada por categoría y la respuesta de menú especial con grupos de modificación |
| `EstadoInventarioService` | Evalúa implicaciones (recetas, comandas activas) antes de cambiar estado de producto o insumo; ejecuta el cambio |
| `MovimientoInventarioService` | Registra ingresos y egresos manuales; actualiza stock de productos e insumos; publica evento WS al tópico `/topic/inventario` |
| `InventarioDescuentoService` | Descuenta stock automáticamente al enviar una comanda a producción, según las recetas |
| `ProductoMapper` | Producto y opciones de modificación → DTOs de carta y menú especial |
| `MovimientoInventarioMapper` | Movimiento de inventario → DTO de respuesta |
...
| DTOs | `CuentaPreliminarResponse`, `CerrarCuentaRequest` |
...
| DTOs | `NotificarCambioRequest`, `AtenderCambioResponse`, `NotificarCambioResponse`, `NotificacionAsistenciaResponse` |
...
| DTOs | `ClienteVentasResponse`, `ClienteVentasResumenResponse`, `VentaAgrupadaAnioResponse`, `VentaAgrupadaMesResponse`, `ClienteBusquedaResponse`, `ClienteListadoResponse` |
